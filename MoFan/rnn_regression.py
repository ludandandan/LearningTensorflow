...
class LSTMRNN(object):
    def __init__(self, n_steps, input_size, output_size, cell_size, batch_size):
        ...
        self.cell_size = cell_size #hidden units size in cell
    ...
    def add_input_layer(self):
        l_in_x = tf.reshape(self.xs, [-1, self.output_size], name='2_2D') # (batch*n_steps, in_size)
    ...
    def mse_error(self, labels, logits):
...
if __name__ == '__main__':
    ...
    plt.show()
    for i in range(200):
        seq, res, xs = get_batch() # xs is time x
        if i==0:
            feed_dict = {
                model.xs: seq,
                model.ys: res
